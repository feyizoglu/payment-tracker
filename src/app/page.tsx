    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-20 pb-16 text-center">
        <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-700 px-4 py-1.5 rounded-full text-sm font-medium mb-6">
          <span>Team payment tracking</span>
        </div>
        <h1 className="text-5xl font-bold text-gray-900 leading-tight mb-6">
          Track every installment,
          <br />
          <span className="text-blue-600">never miss a payment</span>
        </h1>
        <p className="text-xl text-gray-500 max-w-2xl mx-auto mb-10">
          Add credit payments with installment schedules. See exactly which payments
          are due each month — together with your team.
        </p>
        <button
          onClick={() => signIn("google")}
        ...
        {[
          {
            ...
            title: "Add Payments",
            desc: "Log credit payments with amount, start date, and number of installments.",
          },
          {
            icon: <Calendar className="w-6 h-6 text-green-500" />,
            title: "Monthly View",
            desc: "See all due payments grouped by month — past, current, and upcoming.",
          },
          {
            icon: <TrendingDown className="w-6 h-6 text-purple-500" />,
            title: "Track Progress",
            desc: "Mark installments as paid and monitor remaining balance over time.",
          },
          {
            icon: <Users className="w-6 h-6 text-orange-500" />,
            title: "Team Sharing",
            desc: "Create teams and invite members to share and manage payments together.",
          },
        ].map((f) => (
          <div
            key={f.title}
  ...
  return (
    <svg viewBox="0 0 24 24" className="w-5 h-5" aria-hidden="true">
      <path
        ...
        fill={white ? "#fff" : "#4285F4"}
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill={white ? "#ddd" : "#34A853"}
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill={white ? "#ddd" : "#FBBC05"}
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill={white ? "#ddd" : "#EA4335"}
      />
    </svg>
  );
}
